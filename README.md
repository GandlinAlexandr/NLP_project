...
Перед каждым запуском бота обучается модель на этом [датасете](https://drive.google.com/file/d/1hM3IW8FzhQw_5Iz-gPCguN4KiI8bOL7j/view) (внутри кода его имя `df_regions_clean_for_model.p`). По какой-то причине возникает проблема с распаковкой уже готовой модели. Поэтому я решил запускать обчение модели de novo перед запуском бота.
