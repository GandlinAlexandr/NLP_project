...
def get_page(page, DATE):
  ...
  articles = soup.find_all('article', {'class': 'node node--type-news-story node--view-mode-unnews-teaser clearfix'})
  ...
  for ar in articles:
    ...
    try:
      text = ''
      text_list = soup_one.find('div', {'class': 'clearfix text-formatted field field--name-field-text-column field--type-text-long field--label-hidden field__item'}).find_all(['p', 'h2', 'h3']) # Добавление в текст заголовков и подзаголовков самого текста
      for i in text_list:
        text += i.text + '\n'
